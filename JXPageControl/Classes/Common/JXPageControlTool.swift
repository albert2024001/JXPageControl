...
extension UIColor {
    private static func rgbArray(color: UIColor) -> [CGFloat] {
        ...
        if responds(to: #selector(getRed(_:green:blue:alpha:))) {
    ...
    {
        let originArr = rgbArray(color: originColor)
        let targetArr = rgbArray(color: targetColor)
    ...
    {
        let originArr = rgbArray(color: originColor)
        let differenceArr = difference(originColor: originColor,
